...
const CarouselPagination = React.forwardRef<
  HTMLDivElement,
  React.HTMLAttributes<HTMLDivElement> & { selectedBgColor?: string }
>(({ className, selectedBgColor = "bg-primary" }, ref) => {
  const { selectedIndex, slidesCount, scrollTo } = useCarousel();

  return (
    <div ref={ref} className={cn("flex justify-center gap-2 py-4", className)}>
